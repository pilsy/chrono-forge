...
import reducer, { EntitiesState, handleDeleteEntities, handleUpdateEntities } from './utils/entities';
...
export async function EntitiesStateWorkflow({ state = {}, status = 'running', subscriptions = [] }: EntitiesStateWorkflowParams): Promise<void> {
  let iteration = 0;
  ...
  try {
    ...
    setHandler(defineSignal('subscribe'), ({ workflowId, signalName, dataWatchPath }: Subscription) => {
      const currentSubscription = subscriptions.find(
        (s) => s.workflowId === workflowId && s.dataWatchPath === dataWatchPath && s.signalName === signalName
      );
