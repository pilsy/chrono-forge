...
describe('Entities', () => {
  ...
  describe('Helper Functions', () => {
    ...
    describe('createUpdateStatement', () => {
      it('should create an update statement with $merge strategy', () => {
        const state = {
          User: {
            "2": { pid: "2", name: "OldEntity2" },  // This is an existing entity
          },
        };

        const normalizedEntities = {
          User: {
            "2": { pid: "2", name: "UpdatedEntity2" },  // This should trigger a $merge"3": { pid: "3", name: "Entity3" },         // This should trigger a $set
          },
        };

        const expectedStatement = {
          User: {
            "2": { $merge: { name: "UpdatedEntity2", pid: "2" } },  // Existing entity, so $merge is expected"3": { $set: { name: "Entity3", pid: "3" } },           // New entity, so $set is expected
          },
        };
    ...
    it('should create an update statement with $set strategy when entity does not exist', () => {
      const normalizedEntities = {
        User: {
          "3": { pid: "3", name: "Entity3" },
        },
      };
      const expectedStatement = {
        User: {
          $set: {
            "3": { name: "Entity3", pid: "3" },
          },
        },
      };
  ...
  describe('handleUpdateEntities', () => {
    const state = {
      User: {
        2: { pid: 2, name: 'OldEntity2' }, // Pre-existing entity
      },
    };
  ...
  describe('Reducer', () => {
    ...
    beforeEach(() => {
      state = {
        User: {
          "1": { pid: "1", name: 'Entity1' },
          "2": { pid: "2", name: 'Entity2' },
        },
      };
    });

    it('should handle UPDATE_ENTITY', () => {
      const entity = { "1": { pid: "1", name: 'UpdatedEntity1' } };
      const action = updateNormalizedEntity(entity, 'User');
      const newState = reducer(state, action);
      expect(newState).toEqual({
        User: {
          "1": { pid: "1", name: 'UpdatedEntity1' },
          "2": { pid: "2", name: 'Entity2' },
        },
      });
    });
    ...
    it('should handle UPDATE_ENTITIES', () => {
      const updates = {
        User: {
          ...normalize({ pid: "2", name: 'UpdatedEntity2' }, schemas.User).entities.User,
          ...normalize({ pid: "3", name: 'Entity3' }, schemas.User).entities.User,
        }
      };
